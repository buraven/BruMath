...
export function DashboardMetrics({
  ...
  const spentPercentage =
    budget > 0 ? Math.min(100, Math.round((expenses / budget) * 100)) : 0;
  const metrics = [
    {
    ...
    },
    {
      ...
      value: expenses,
      detail: `${spentPercentage}% do orçamento`,
  ...
  return (
    <section className={styles.grid} aria-label="Resumo financeiro do mês">
      {metrics.map((metric) => (
        <article className={styles.card} key={metric.label}>
          <div className={styles.heading}>
            <span>{metric.label}</span>
            <i className={styles[metric.tone]} aria-hidden="true">
              {metric.icon}
            </i>
          </div>
