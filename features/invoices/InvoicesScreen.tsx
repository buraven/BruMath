    ...
    <section className={styles.screen} aria-labelledby="invoices-title">
      <header className={styles.header}>
        <div>
          <span className={styles.eyebrow}>
            <CreditCard size={15} /> Cartões e pagamentos
          </span>
      ...
      {visible.length ? (
        ...
        <div className={styles.empty}>
          ...
          <p>
            {props.cards.length
              ? "Compras vinculadas a cartões aparecem conforme o ciclo de fechamento."
              : "Cadastre um cartão para começar a acompanhar faturas reais."}
          </p>
          <button
            type="button"
  ...
  return (
    <article className={styles.invoiceCard}>
      <div className={styles.invoiceHead}>
        ...
        <span className={invoice.status === "paid" ? styles.paid : styles.open}>
          {invoice.status === "paid" ? "Paga" : "Aberta"}
        </span>
      </div>
      <div className={styles.invoiceInfo}>
        <span>
          Fecha em <strong>{formatDate(invoice.closingDate)}</strong>
        </span>
        <span>
          Vence em <strong>{formatDate(invoice.dueDate)}</strong>
        </span>
        <span>
          Valor atual <strong>{formatMoney(invoice.total)}</strong>
        </span>
      </div>
      ...
      <div className={styles.detailGrid}>
        <aside className={styles.invoiceSummary}>
          ...
          {invoice.status === "open" ? (
            ...
            <p className={styles.paid}>Paga em {formatDate(invoice.dueDate)}</p>
          )}
        </aside>
