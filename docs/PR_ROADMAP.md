...
nenhuma dessas implementações de persistência; os PRs desta fase não devem introduzir
novo acoplamento ao `localStorage`.
...
| #54 | `feature/invoices-cards-pr54`               | Faturas & Cartões: cartões configuráveis, ciclos determinísticos e pagamentos sem duplicar gastos. **Concluído e integrado no merge `2770c59`.**                                                                                                                                                                                                                                                                                                          |
| #55 | `feature/calendar-commitments-pr55`         | Calendário & Compromissos. **Concluído.** Projeta transações e compromissos existentes sem fonte de verdade paralela; validado por testes automatizados, CI Playwright, Preview Vercel, aceite visual manual em tablet/iPad e reconciliação da projeção financeira sem duplicar Hocks.                                                                                                                                                                    |
| #56 | —                                           | Financial Guardrails: provenance financeira, validação e classificação de respostas como fact, calculation, simulation ou inference; serviços determinísticos permanecem fonte de verdade e nenhuma mutação financeira ocorre sem confirmação. **Concluído.**                                                                                                                                                                                             |
| #57 | `feature/supabase-persistence-pr57`         | Supabase Persistence: persistência centralizada gradual em Supabase/PostgreSQL via `FinancialDataSource` / `SupabaseFinancialDataSource`, schema, repositories/adapters, autenticação, RLS, importação segura do `localStorage`, compatibilidade e tratamento de erros/conflitos. **Concluído e validado; aguardando merge.** Inclui Magic Link, hidratação/reconciliação remota, fila/retry/flush, QA de logout/login e alinhamento de escopo de perfil. |
| #58 | —                                           | Conversation Context: follow-ups, referências, memória curta e contexto conversacional limitado e resumido.                                                                                                                                                                                                                                                                                                                                               |
| #59 | —                                           | Token & Cost Optimization: seleção mínima de contexto, cache seguro, resumo de histórico, roteamento/model selection por complexidade e deterministic-first.                                                                                                                                                                                                                                                                                              |
...
### Etapa intermediária antes do #58

Revisar a UX e a semântica do seletor de visão financeira, sem reinterpretar
dados existentes: separar a propriedade/responsabilidade do lançamento
(`Bruna` / `Matheus` / `Casal`) do filtro de visualização (`Todos` / `Bruna` /
`Matheus`). Esta etapa ainda não possui número de PR nem implementação iniciada.
...
### PR #56 — Financial Guardrails (concluído)

Classificar cada resposta financeira como `fact`, `calculation`, `simulation` ou
`inference`, com proveniência e validação compatíveis. Serviços determinísticos
continuam sendo a fonte de verdade; inferências não são apresentadas como fatos e
nenhuma mutação financeira ocorre sem preview e confirmação explícita.

### PR #57 — Supabase Persistence (concluído e validado; aguardando merge)
...
O PR entrega projeto Supabase/PostgreSQL, schema financeiro, repositories e adapters,
Magic Link, Row Level Security, migração explícita e idempotente de `localStorage`,
hidratação/reconciliação remota, fila serializada com retry/flush e compatibilidade de
transição. O QA validou logout/login, leitura e mutações remotas, persistência após
reautenticação e a consistência de escopo Bruna/Matheus/Casal. O marco após o #57 é o
BruMath apto a iniciar uso com dados financeiros reais persistidos centralmente, sujeito
ao merge deste PR. O `localStorage` não é removido imediatamente: a transição permanece
gradual e compatível.
...
Conversational AI, Guardrails, persistência centralizada, Conversation Context, otimizações
de custo e multiusuário/sincronização. Home e Assistente deverão consumir o mesmo motor
