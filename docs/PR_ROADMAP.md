...
| #56 | —                                           | Financial Guardrails: provenance financeira, validação e classificação de respostas como fact, calculation, simulation ou inference; serviços determinísticos permanecem fonte de verdade e nenhuma mutação financeira ocorre sem confirmação.                                                                                                                                                                                                                          |
| #57 | `feature/supabase-persistence-pr57`         | Supabase Persistence: persistência centralizada gradual em Supabase/PostgreSQL via `FinancialDataSource` / `SupabaseFinancialDataSource`, schema, repositories/adapters, autenticação, RLS, importação segura do `localStorage`, compatibilidade e tratamento de erros/conflitos. **Em implementação.** Marco: BruMath apto a iniciar uso com dados financeiros reais persistidos centralmente, condicionado aos critérios de segurança, migração e validação deste PR. |
...
### PR #56 — Financial Guardrails (futuro)
...
conflitos e recuperação de erros, com testes. O marco após o #57 é o BruMath apto a
iniciar uso com dados financeiros reais persistidos centralmente, condicionado aos
critérios de segurança, migração e validação deste PR. O `localStorage` não é removido
imediatamente: a transição permanece gradual e compatível.
