...
| #57 | —                                           | Supabase Persistence: persistência centralizada gradual em Supabase/PostgreSQL via `FinancialDataSource` / `SupabaseFinancialDataSource`, schema, repositories/adapters, autenticação, RLS, importação segura do `localStorage`, compatibilidade e tratamento de erros/conflitos. Marco: BruMath apto a iniciar uso com dados financeiros reais persistidos centralmente, condicionado aos critérios de segurança, migração e validação deste PR. |
...
### PR #57 — Supabase Persistence (futuro)
